    ...
    $html = '<table class="table">
      ...
      <table class="table">
        <tbody>
          ...
          <tr>
            <td class="font_500"><h4>'. _l('rq_description').':</h4></td>
      ...
      $html .=  '<table class="table pur_request-item">
            <thead>
              <tr class="border_tr">
                <th align="left" class="width20 thead-dark">'._l('items').'</th>
                <th align="right" class="thead-dark width20">Nhà cung cấp</th>
...
foreach($pur_request_detail as $row){
  ...
  if($items){
      ...
      $html .= '<tr class="border_tr">
          <td class="width20" align="left">'.$items->commodity_code.' - '.$items->description.'</td>
          <td class="width20">'.$row['supplier'].'</td>
          <td class="width15" align="right">'.app_format_money(floor($row['unit_price']), $base_currency->symbol).'</td>
          <td class="width15" align="right">'.app_format_number($row['quantity'],'').' '.$unit_name.'</td>
          <td class="width15" align="right">'.app_format_money(floor($row['into_money']), $base_currency->symbol).'</td>';
    
              $html .= '<td class="width10" align="right">'.app_format_money(floor($row['tax_value']), $base_currency->symbol).'</td>';
          
          $html .= '<td class="width15" align="right">'.app_format_money(floor($row['total']), $base_currency->symbol).'</td>
        </tr>';
  }else{
      $unit_name = isset($units->unit_name) ? $units->unit_name : '';
      $html .= '<tr class="border_tr">
  ...
  $tmn += floor($row['into_money']);
  $_total += floor($row['total']);
}  
$html .=  '</tbody>
</table><br><br>';
...
$html .= '<tr>
           ...
           <td>'. _l('subtotal').'</td>
           <td class="subtotal">
              '. app_format_money(floor($tmn), $base_currency->symbol).'
           </td>
        </tr>';
...
$html .= '<tr>
           ...
           <td>'. _l('total').'</td>
           <td class="subtotal">
              '. app_format_money(floor($_total), $base_currency->symbol).'
           </td>
        </tr>';
$html .= ' </tbody></table>';
$html .= '<br>
...
<table class="table">
  <tbody>
    <tr>';
/**  ẩn chữ ký ng yêu cầu và tên phòng mua hàng        
...
if(count($list_approve_status) > 0){
    $titles = ['TRƯỞNG BỘ PHẬN', 'PHÒNG MUA HÀNG', 'TỔNG GIÁM ĐỐC'];
    $title_index = 0;
    
    foreach ($list_approve_status as $value) {
        $html .= '<td class="td_appr">';
        // Thêm tiêu đề cho mỗi cột
        if($title_index < count($titles)) {
            $html .= '<h4 style="margin-bottom: 10px;">' . $titles[$title_index] . '</h4>';
            $title_index++;
        }
...
$html .= '</tr></tbody></table>';
$html .= '<style>
    .td_appr { text-align: center; }
    .td_appr h4 { font-weight: bold; }
</style>';
$html .= '<link href="' . FCPATH.'modules/purchase/assets/css/pur_order_pdf.css' . '"  rel="stylesheet" type="text/css" />';
return $html;
    }
    ...
    $html = '<table class="table">
      ...
      $html .=  '<table class="table pur_request-item">
            <thead>
              <tr class="border_tr">
                <th align="left" class="thead-dark width20">'._l('items').'</th>
                <th align="right" class="thead-dark width20">Nhà cung cấp</th>
...
/**
    ...
    */
     ...
     */
    public function create_purchase_request_row_template($name = '', $item_code = '', $item_text = '', $unit_price = '', $quantity = '', $unit_name = '', $unit_id = '', $into_money = '', $item_key = '', $tax_value = '', $total = '', $tax_name = '', $tax_rate = '', $tax_id = '', $is_edit = false, $currency_rate = 1, $to_currency = '', $supplier=''){
        ...
        //quang
        $row .= '<td class="">' . render_input($name_supplier, '', $supplier, [] ) . '</td>';
        //quang
    ...
    public function mapDepartmentsWithKey($departments) {
        $data = [];
        foreach($departments as $item) {
            $data[$item['departmentid']] = $item;
        }
        return $data;
    }
}