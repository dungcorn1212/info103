...
function pur_get_item_row_template(name, item_code, item_text, unit_price, quantity, unit_name, unit_id, into_money, item_key, tax_value, total, taxname, currency_rate, to_currency, supplier)  {
  "use strict";
...
function pur_add_item_to_table(data, itemid) {
  ...
  pur_get_item_row_template('newitems[' + item_key + ']', data.item_code, data.item_text,data.unit_price,data.quantity, data.unit_name, data.unit_id, data.into_money, item_key, data.tax_value, data.total, data.taxname, currency_rate, to_currency, data.supplier).done(function(output){
    table_row += output;
...
function pur_get_item_preview_values() {
  ...
  response.supplier = $('.invoice-item .main input[name="supplier"]').val();
  return response;
}
