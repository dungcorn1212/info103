			...
			<div class="col-md-6">
				<div class="card">
					<div class="card-body">
                        ...
                        <h6 class="card-title bold duration-title <?php if(strtotime(date("Y-m-d")) > strtotime($rec_campaingn->cp_to_date)){echo 'text-danger';}else{echo 'text-muted';} ?>">
    <?php echo _l('duration') ?>: 
    <?php 
        // Định dạng lại ngày tháng từ Y-m-d sang d/m/Y
        $from_date = date("d/m/Y", strtotime($rec_campaingn->cp_from_date));
        $to_date = date("d/m/Y", strtotime($rec_campaingn->cp_to_date));
        echo html_entity_decode($from_date . ' - ' . $to_date); 
    ?>
</h6>
			...
			<div class="col-md-12 ">
				<div class="row button-action-row">
					...
					<div class="card text-right buton-margin">
						<div class="card-body">
							<a href="<?php echo site_url('recruitment/recruitment_portal') ?>" class="btn btn-default appointment_go_back"><i class="fa fa-home"></i> <?php echo _l('go_back_tuyen_dung') ?></a>
						</div>
					</div>
				</div>
			</div>
