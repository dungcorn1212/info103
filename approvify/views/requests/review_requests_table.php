...
$aColumns = [
    ...
    '(SELECT created_at FROM ' . db_prefix() . 'approvify_request_activity WHERE request_id = ' . db_prefix() . 'approvify_requests.id ORDER BY id DESC LIMIT 1) as last_activity_time',
...
if (!in_array(get_staff_user_id(), [1, 6])) {
...
if (isset($postData['approvify_last_reviewer']) && $postData['approvify_last_reviewer']) {
    ...
    $select[] = '(SELECT created_at FROM ' . db_prefix() . 'approvify_request_activity WHERE request_id = ' . db_prefix() . 'approvify_requests.id ORDER BY id DESC LIMIT 1) as last_update_time';
}

// Add date range filter for both creation and last update
if (isset($postData['approvify_date_from']) && $postData['approvify_date_from']) {
    array_push($where, 'AND (' . db_prefix() . 'approvify_requests.created_at >= "' . to_sql_date($postData['approvify_date_from']) . '" OR (SELECT created_at FROM ' . db_prefix() . 'approvify_request_activity WHERE request_id = ' . db_prefix() . 'approvify_requests.id ORDER BY id DESC LIMIT 1) >= "' . to_sql_date($postData['approvify_date_from']) . '")');
}

if (isset($postData['approvify_date_to']) && $postData['approvify_date_to']) {
    array_push($where, 'AND (' . db_prefix() . 'approvify_requests.created_at <= "' . to_sql_date($postData['approvify_date_to']) . ' 23:59:59" OR (SELECT created_at FROM ' . db_prefix() . 'approvify_request_activity WHERE request_id = ' . db_prefix() . 'approvify_requests.id ORDER BY id DESC LIMIT 1) <= "' . to_sql_date($postData['approvify_date_to']) . ' 23:59:59")');
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if (!empty($aRow['last_reviewer_id'])) {
            ...
            if (!empty($aRow['last_activity_time'])) {
                $last_activity_time = date('d/m/Y H:i:s', strtotime($aRow['last_activity_time']));
            }
        }

        $row[] = $last_reviewer_name . (!empty($last_activity_time) ? '<br><small>' . $last_activity_time . '</small>' : '');
